...
from config import SUPPORT_CHAT_ID, SUPER_ADMIN_IDS, REFERRAL_BONUS_RUB
from database.db import (
    ...
    get_pilot_class, get_pilot_achievements, get_referral_stats,
...
from keyboards.profile_menu import profile_menu, build_profile_menu
...
from services.referrals import process_referral_registration
...
async def start(message: Message, state: FSMContext):
    ...
    if len(parts) > 1 and parts[1].startswith("ref_"):
        try:
            ...
            if candidate != message.from_user.id and await get_pilot_by_telegram_id(candidate):
...
async def registration_phone(message: Message, state: FSMContext):
    ...
    referrer_id = reg_data.get("referrer_id")
    await state.clear()
    ...
    if referrer_id:
        referral_task = asyncio.create_task(
            process_referral_registration(message.from_user.id, referrer_id, message.bot)
        )
        _background_tasks.add(referral_task)
        referral_task.add_done_callback(_background_tasks.discard)
...
async def referral_invite(message: Message):
    pilot = await get_pilot_by_telegram_id(message.from_user.id)
    if not pilot:
...
async def profile(message: Message, state: FSMContext):
    try:
        ...
        if text is None:
            ...
            return
        enabled = await get_standings_notify_enabled(message.from_user.id)
        await message.answer(text, reply_markup=build_profile_menu(enabled))
    except Exception as e:
...
async def refresh_profile(callback: CallbackQuery):
    try:
        ...
        try:
            ...
            await callback.message.edit_text(text, reply_markup=build_profile_menu(enabled))
...
async def toggle_standings_notify(callback: CallbackQuery):
    try:
        ...
        await set_standings_notify_enabled(callback.from_user.id, new_value)
        try:
            await callback.message.edit_reply_markup(reply_markup=build_profile_menu(new_value))
        except Exception:
            pass
