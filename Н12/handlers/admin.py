...
from keyboards.admin_menu import admin_menu, get_admin_menu
...
def is_admin(uid): return uid in SUPER_ADMIN_IDS

def is_staff(uid): return uid in set(ADMIN_IDS) | set(SUPER_ADMIN_IDS)
...
class ChangePilotPhone(StatesGroup):
    phone = State()
...
async def admin_panel(message: Message):
    if not is_staff(message.from_user.id):
        ...
        return
    await message.answer("🛠 <b>Панель администратора</b>", reply_markup=get_admin_menu(message.from_user.id))
...
async def change_phone(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        ...
        return
    tid = int(callback.data.split("_")[1])
    await state.update_data(change_phone_user=tid)
    await state.set_state(ChangePilotPhone.phone)
    await callback.message.edit_text("📱 Введите новый номер телефона:")


@router.message(ChangePilotPhone.phone)
async def save_new_phone(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    data = await state.get_data()
    tid = data.get("change_phone_user")
    phone = normalize_phone_for_bot(message.text or "")
    if tid is None or not phone:
        await message.answer("❌ Введите корректный российский номер телефона.")
        return
    db = await get_db()
    try:
        cur = await db.execute("SELECT telegram_id FROM pilots WHERE phone = ?", (phone,))
        row = await cur.fetchone()
        if row and int(row[0]) != int(tid):
            await message.answer("❌ Этот телефон уже привязан к другому пилоту.")
            return
        cur = await db.execute("UPDATE pilots SET phone = ? WHERE telegram_id = ?", (phone, tid))
        await db.commit()
        if cur.rowcount <= 0:
            await message.answer("❌ Пилот не найден.")
            return
        ...
        await db.close()
    await state.clear()
    await message.answer(f"✅ Телефон пилота обновлён: {phone}")
    try:
        await message.bot.send_message(tid, f"📱 Администратор обновил ваш телефон: {phone}")
    except Exception:
        pass
...
async def send_notifications(bot, discipline, new_username, lap_text, selected_tid, track, group_id, promoted_to=None):
    ...
    try:
        ...
        logger.exception("Не удалось обновить TOP-5 уведомления")
