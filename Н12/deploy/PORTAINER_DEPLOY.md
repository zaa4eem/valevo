...
1 GB RAM на весь сервер (по факту `free -h` на этом сервере показал ~702 МБ
total — часть уходит гипервизору, это нормально) — тариф тесный, особенно с
несколькими ботами. Ориентировочно:
...
| valevo-tvboard (FastAPI/uvicorn) | 60–100 МБ |

Итого под один этот проект — реалистично 610–880 МБ уже занято на 702 МБ
физических. Своп (ниже) обязателен — без него это не «медленно», а
падения по OOM. Свободного места под ещё одного полноценного бота на
этом тарифе практически нет.
...
Мини-апп и ТВ-табло разведены по **поддоменам**, не по путям одного домена —
оба приложения (FastAPI) считают, что работают в корне сайта, путь вроде
`/app` потребовал бы переписывать роутинг и все ссылки на статику в обоих.
Поддомен работает "из коробки" — тот же контейнер, просто другой домен.

В панели reg.ru (там же, где сейчас `ns1.hosting.reg.ru` для старого
shared-хостинга) добавить две A-записи на IP этого сервера:

| Поддомен | Значение |
|---|---|
| `app.valivo.online` | IP сервера |
| `board.valivo.online` | IP сервера |

**Именно A-записи на поддомены, не трогая NS** — домен целиком переносить
не нужно. Подождите распространения DNS (обычно от нескольких минут до
часа) перед шагом 5 — `acme-companion` не выпустит сертификаты, если
домены ещё не резолвятся на сервер.
...
cd valevo/Н12
cp .env.example .env
nano .env   # заполнить BOT_TOKEN, ADMIN_IDS, WEBAPP_BASE_URL=https://app.valivo.online,
            # WEBAPP_DOMAIN=app.valivo.online, TVBOARD_DOMAIN=board.valivo.online,
...
```bash
curl -I https://app.valivo.online      # должен вернуть 200, с валидным сертификатом
curl -I https://board.valivo.online    # то же самое для табло
docker compose logs valevo-webapp | tail -30
docker compose logs valevo-tvboard | tail -30
docker compose logs valevo-bot | tail -30
```

В Telegram: `@BotFather → /mybots → [бот] → Bot Settings → Menu Button` —
указать `https://app.valivo.online`, если ещё не настроено (или это
сделает сам бот при старте — см. `main.py:_configure_menu_button`, он это
