...
from data.tournament import canonical_class_name
...
async def init_db():
    ...
    await db.execute("""
        CREATE TABLE IF NOT EXISTS referrals (
            ...
            bonus_rub REAL NOT NULL DEFAULT 350,
    ...
    await db.execute("""
        CREATE TABLE IF NOT EXISTS roulette_spins (
            ...
            prize_status TEXT NOT NULL,
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    """)
    await db.execute("CREATE INDEX IF NOT EXISTS idx_roulette_spins_user ON roulette_spins(telegram_id, created_at)")
    ...
    await db.execute("""
        CREATE TABLE IF NOT EXISTS standings_notify_v3 (
            telegram_id INTEGER NOT NULL,
            month_key TEXT NOT NULL,
            scope TEXT NOT NULL DEFAULT 'overall',
            notified_place INTEGER, notified_total REAL, notified_at TEXT,
            pending_place INTEGER, pending_total REAL, pending_rival TEXT,
            pending_since TEXT, sent_day TEXT, sent_today INTEGER NOT NULL DEFAULT 0,
            PRIMARY KEY (telegram_id, month_key, scope)
        )
    """)
    await db.execute("CREATE INDEX IF NOT EXISTS idx_standings_v3_pending ON standings_notify_v3(month_key, pending_since)")
...
async def create_pilot(telegram_id, username, phone, yclients_client_id=None):
    ...
    try:
        ...
        cursor = await db.execute(
            '''INSERT OR IGNORE INTO pilots (telegram_id, username, phone, pilot_number, yclients_client_id, notify_standings)
               VALUES (?, ?, ?, ?, ?, 1)''',
...
async def get_all_disciplines():
    db = await get_db()
    try:
        cursor = await db.execute("SELECT name FROM disciplines ORDER BY id")
        return list(dict.fromkeys(canonical_class_name(row[0]) for row in await cursor.fetchall()))
    finally:
        await db.close()


async def get_tracks_for_discipline(discipline: str):
    discipline = canonical_class_name(discipline)
    db = await get_db()
    try:
        cursor = await db.execute("SELECT d.name, t.name FROM tracks t JOIN disciplines d ON d.id = t.discipline_id ORDER BY t.id")
        return list(dict.fromkeys(track for name, track in await cursor.fetchall() if canonical_class_name(name) == discipline))
    finally:
        await db.close()
...
async def get_all_class_benchmarks(month_key: str) -> dict[str, dict]:
    ...
    # Явно заданный эталон канонического класса имеет приоритет над алиасом.
    records.sort(key=lambda row: row["class_name"] == canonical_class_name(row["class_name"]))
    for row in records:
        name = canonical_class_name(row["class_name"])
        results[name] = {**row, "class_name": name}
    return results
...
async def claim_pilot_promotion(telegram_id: int, current_class: str, target: str,
                                month_key: str, rating_bonus: int) -> bool:
    """Смена ступени и награда одной транзакцией, в том числе при двух админах."""
    db = await get_db()
    try:
        await db.execute("BEGIN IMMEDIATE")
        await db.execute(
            "INSERT OR IGNORE INTO pilot_class_status(telegram_id) VALUES (?)",
            (telegram_id,),
        )
        cursor = await db.execute(
            """UPDATE pilot_class_status SET current_class = ?, promoted_at = CURRENT_TIMESTAMP,
               promoted_month_key = ?, updated_at = CURRENT_TIMESTAMP
               WHERE telegram_id = ? AND current_class = ?""",
            (target, month_key, telegram_id, current_class),
        )
        if cursor.rowcount != 1:
            await db.rollback()
            return False
        await db.execute("UPDATE pilots SET rating = rating + ? WHERE telegram_id = ?",
                         (rating_bonus, telegram_id))
        await db.commit()
        return True
    except Exception:
        await db.rollback()
...
async def get_pilot_month_best(telegram_id: int, discipline: str, month_start_iso: str, month_end_iso: str) -> tuple[int | None, int]:
    bests = await get_all_month_bests(month_start_iso, month_end_iso)
    return bests.get((telegram_id, canonical_class_name(discipline)), (None, 0))


async def get_month_participants(discipline: str, month_start_iso: str, month_end_iso: str) -> list[dict]:
    bests = await get_all_month_bests(month_start_iso, month_end_iso)
    name = canonical_class_name(discipline)
    return [{"telegram_id": tid, "best_ms": best, "starts": starts}
            for (tid, cls), (best, starts) in bests.items() if cls == name]
...
async def get_all_month_bests(month_start_iso: str, month_end_iso: str) -> dict[tuple[int, str], tuple[int | None, int]]:
    ...
    for telegram_id, discipline, best_ms, starts in rows:
        key = (int(telegram_id), canonical_class_name(discipline))
        old_best, old_starts = bests.get(key, (None, 0))
        best = int(best_ms) if best_ms is not None else None
        if old_best is not None:
            best = min(old_best, best) if best is not None else old_best
        bests[key] = (best, old_starts + int(starts or 0))
    return bests


async def get_month_tournament_bests(start_iso: str, end_iso: str, benchmarks: dict) -> tuple[dict, dict]:
    """Лучшие круги сезона на трассе эталона (или актуальной трассе дисциплины).

    Возвращает словарь {(pilot, class): (best_ms, starts)} и трассы классов.
    Результаты другой трассы не могут пройти эталон за счёт меньшей длины круга.
    """
    db = await get_db()
    try:
        cursor = await db.execute(
            """SELECT l.telegram_id, d.name, l.track, MIN(l.lap_time_ms), COUNT(*),
                      MAX(l.created_at), MAX(l.id)
               FROM laps l JOIN disciplines d ON d.id = l.discipline_id
               WHERE l.created_at >= ? AND l.created_at < ?
                 AND l.telegram_id IS NOT NULL AND l.lap_time_ms > 0
               GROUP BY l.telegram_id, d.name, l.track""", (start_iso, end_iso),
        )
        rows = await cursor.fetchall()
    finally:
        await db.close()
    latest = {}
    for tid, name, track, best, starts, created, lap_id in rows:
        name = canonical_class_name(name)
        if (created, lap_id) > latest.get(name, ("", 0, ""))[:2]:
            latest[name] = (created, lap_id, str(track or "").strip())
    tracks = {name: values[2] for name, values in latest.items()}
    for name, benchmark in benchmarks.items():
        if benchmark.get("track") and str(benchmark["track"]).strip():
            tracks[name] = str(benchmark["track"]).strip()
    bests = {}
    for tid, name, track, best, starts, _created, _lap_id in rows:
        name = canonical_class_name(name)
        if str(track or "").strip().casefold() != tracks.get(name, "").casefold():
            continue
        key = (int(tid), name)
        old_best, old_starts = bests.get(key, (None, 0))
        bests[key] = (min(int(best), old_best) if old_best is not None else int(best), old_starts + int(starts))
    return bests, tracks
...
# --- Уведомления общего зачёта и отдельных дисциплин ---
async def get_standings_notify_state(month_key: str, scope: str = "overall") -> dict[int, dict]:
    db = await get_db()
    try:
        cursor = await db.execute("SELECT * FROM standings_notify_v3 WHERE month_key = ? AND scope = ?", (month_key, scope))
        columns = [col[0] for col in cursor.description]
        return {row[0]: dict(zip(columns, row)) for row in await cursor.fetchall()}
    finally:
        await db.close()


async def set_standings_baseline(month_key: str, places: dict, scope: str = "overall") -> None:
    db = await get_db()
    try:
        await db.execute("BEGIN IMMEDIATE")
        await db.execute("DELETE FROM standings_notify_v3 WHERE month_key = ? AND scope = ?", (month_key, scope))
        await db.executemany(
            """INSERT INTO standings_notify_v3 (telegram_id, month_key, scope, notified_place, notified_total)
               VALUES (?, ?, ?, ?, ?)""",
            [(tid, month_key, scope, place, total) for tid, (place, total) in places.items()],
        )
        await db.execute(
            "INSERT OR REPLACE INTO bot_settings(key, value) VALUES (?, '1')",
            (f"standings_v3_baseline:{month_key}:{scope}",),
        )
        await db.commit()
    finally:
        await db.close()
...
async def queue_standings_change(telegram_id: int, month_key: str, place: int | None,
                                total: float, rival: str | None, scope: str = "overall") -> None:
    db = await get_db()
    try:
        await db.execute(
            """INSERT INTO standings_notify_v3
                   (telegram_id, month_key, scope, pending_place, pending_total, pending_rival, pending_since)
               VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
               ON CONFLICT(telegram_id, month_key, scope) DO UPDATE SET
                   pending_place = excluded.pending_place, pending_total = excluded.pending_total,
                   pending_rival = excluded.pending_rival,
                   pending_since = COALESCE(standings_notify_v3.pending_since, CURRENT_TIMESTAMP)""",
            (telegram_id, month_key, scope, place, total, rival),
        )
        await db.commit()
    finally:
        await db.close()
...
async def get_due_standings_changes(month_key: str) -> list[dict]:
    ...
    try:
        cursor = await db.execute(
            "SELECT * FROM standings_notify_v3 WHERE month_key = ? AND pending_since IS NOT NULL ORDER BY pending_since, scope, telegram_id",
            (month_key,),
        )
        columns = [col[0] for col in cursor.description]
...
async def clear_standings_pending(telegram_id: int, scope: str = "overall") -> None:
    db = await get_db()
    try:
        await db.execute(
            """UPDATE standings_notify_v3 SET pending_place = NULL, pending_total = NULL,
               pending_rival = NULL, pending_since = NULL WHERE telegram_id = ? AND scope = ?""",
            (telegram_id, scope),
        )
        await db.commit()
    finally:
...
async def mark_standings_notified(telegram_id: int, place: int | None, total: float,
                                 day_key: str, scope: str = "overall", *, delivered: bool = True) -> None:
    db = await get_db()
    try:
        if not delivered:
            # Выключенный или заблокировавший бота пользователь: обновляем
            # точку сравнения без фиктивного расхода лимита доставок.
            await db.execute(
                """UPDATE standings_notify_v3 SET notified_place = ?, notified_total = ?,
                   pending_place = NULL, pending_total = NULL, pending_rival = NULL, pending_since = NULL
                   WHERE telegram_id = ? AND scope = ?""",
                (place, total, telegram_id, scope),
            )
            await db.commit()
            return
        await db.execute(
            """UPDATE standings_notify_v3
               SET notified_place = ?, notified_total = ?, notified_at = CURRENT_TIMESTAMP,
                   pending_place = NULL, pending_total = NULL, pending_rival = NULL, pending_since = NULL,
                   sent_today = CASE WHEN sent_day = ? THEN sent_today + 1 ELSE 1 END, sent_day = ?
               WHERE telegram_id = ? AND scope = ?""",
            (place, total, day_key, day_key, telegram_id, scope),
        )
        await db.commit()
    finally:
        await db.close()
...
async def reset_standings_state_for_new_month(month_key: str) -> None:
    db = await get_db()
    try:
        await db.execute("DELETE FROM standings_notify_v3 WHERE month_key != ?", (month_key,))
        await db.commit()
    finally:
        await db.close()
...
# --- Referrals ---
async def create_referral(referrer_id: int, referred_id: int, bonus_rub: float) -> int | None:
    if int(referrer_id) == int(referred_id):
        return None
    db = await get_db()
    try:
        cur = await db.execute(
            "INSERT OR IGNORE INTO referrals(referrer_id, referred_id, bonus_rub) VALUES (?, ?, ?)",
            (int(referrer_id), int(referred_id), float(bonus_rub)),
        )
        await db.commit()
        if not cur.rowcount:
            return None
...
async def get_referral(referral_id: int) -> dict | None:
    db = await get_db()
    try:
        cur = await db.execute(
            "SELECT id, referrer_id, referred_id, bonus_rub, referrer_rewarded, referred_rewarded, created_at, rewarded_at FROM referrals WHERE id = ?",
            (int(referral_id),),
        )
        row = await cur.fetchone()
        if not row:
            return None
        keys = ["id","referrer_id","referred_id","bonus_rub","referrer_rewarded","referred_rewarded","created_at","rewarded_at"]
        return dict(zip(keys,row))
    finally:
        await db.close()

async def mark_referral_rewarded(referral_id: int, side: str) -> None:
    column = "referrer_rewarded" if side == "referrer" else "referred_rewarded"
    db = await get_db()
    try:
        await db.execute(f"UPDATE referrals SET {column} = 1 WHERE id = ?", (int(referral_id),))
        await db.execute(
            "UPDATE referrals SET rewarded_at = CURRENT_TIMESTAMP WHERE id = ? AND referrer_rewarded = 1 AND referred_rewarded = 1",
            (int(referral_id),),
        )
        await db.commit()
    finally:
...
async def get_referral_stats(telegram_id: int) -> dict:
    ...
    try:
        cur = await db.execute(
            "SELECT COUNT(*), COALESCE(SUM(CASE WHEN referrer_rewarded = 1 THEN bonus_rub ELSE 0 END),0) FROM referrals WHERE referrer_id = ?",
            (int(telegram_id),),
        )
        invited, earned = await cur.fetchone()
        return {"invited": int(invited or 0), "earned": float(earned or 0)}
    finally:
        await db.close()


# --- Roulette ---
async def record_roulette_spin(telegram_id: int, cost_rub: float, prize_code: str, prize_kind: str, prize_value: float, prize_status: str) -> int:
    db = await get_db()
    try:
        cur = await db.execute(
            "INSERT INTO roulette_spins(telegram_id,cost_rub,prize_code,prize_kind,prize_value,prize_status) VALUES (?,?,?,?,?,?)",
            (int(telegram_id), float(cost_rub), prize_code, prize_kind, float(prize_value), prize_status),
        )
        await db.commit()
        return cur.lastrowid
    finally:
...
async def get_pilot_spin_history(telegram_id: int, limit: int = 50) -> list[dict]:
    db = await get_db()
    try:
        cur = await db.execute(
            "SELECT id,cost_rub,prize_code,prize_kind,prize_value,prize_status,created_at FROM roulette_spins WHERE telegram_id=? ORDER BY id DESC LIMIT ?",
            (int(telegram_id), int(limit)),
        )
        rows = await cur.fetchall()
        keys=["id","cost_rub","prize_code","prize_kind","prize_value","prize_status","created_at"]
